...
from msibi.utils.corrections import _pair_head_correction, _pair_tail_correction
...
def test_head_correction():
    ...
    linear_V = _pair_head_correction(r, np.copy(V), V_prev, "linear")
    assert not np.isnan(linear_V).any() and not np.isinf(linear_V).any()
    assert all(linear_V[cutoff:] == V[cutoff:])

    exp_V = _pair_head_correction(r, np.copy(V), V_prev, "exponential")
